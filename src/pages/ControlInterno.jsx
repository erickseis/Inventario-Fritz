...
const demoLists = [
  {
    ...
    items_completados: 0,
    items: [
      { id: 11, nombre: 'Cajas de Electrónicos', cantidad_meta: 200, cantidad_cargada: 0 },
      { id: 12, nombre: 'Paquetes de Ropa', cantidad_meta: 150, cantidad_cargada: 0 },
      { id: 13, nombre: 'Muebles de Oficina', cantidad_meta: 50, cantidad_cargada: 0 },
    ],
  },
  {
    id: 2,
    ...
    items_completados: 0,
    items: [
      { id: 21, nombre: 'Contenedores Plásticos', cantidad_meta: 80, cantidad_cargada: 20 },
      { id: 22, nombre: 'Manual de Usuario', cantidad_meta: 120, cantidad_cargada: 40 },
      { id: 23, nombre: 'Pallets de Exhibición', cantidad_meta: 60, cantidad_cargada: 0 },
    ],
  },
];
...
const Modal = ({ title, show, onClose, footer, children }) => {
  if (!show) return null;
  return (
    <div className="modal-backdrop fade show" style={{ display: 'block', backgroundColor: 'rgba(0, 0, 0)' }}>
      <div className="modal d-block" tabIndex="-1" role="dialog">
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content" style={{ backgroundColor: '#ffffff', color: '#212529' }}>
            <div className="modal-header" style={{ backgroundColor: '#ffffff', borderBottom: '1px solid #dee2e6' }}>
              <h5 className="modal-title" style={{ color: '#212529' }}>{title}</h5>
              <button type="button" className="btn-close" onClick={onClose} aria-label="Cerrar"></button>
            </div>
            <div className="modal-body" style={{ backgroundColor: '#ffffff', color: '#212529' }}>
              {children}
            </div>
            {footer && (
...
const ControlInterno = () => {
    ...
    async (listIdToSelect = null) => {
      if (demoMode) {
        ...
        setLists((prev) => {
          ...
          } else if (!selectedListId || !currentLists.some((l) => l.id === selectedListId)) {
      ...
      try {
        ...
        if (data.length === 0) {
          ...
          setSelectedListId(listIdToSelect);
        } else if (!selectedListId || !data.some((l) => l.id === selectedListId)) {
  ...
  const handleInputChange = (itemId, value) => {
    ...
    const clamped = numeric === '' ? '' : Math.min(999999, parseInt(numeric, 10));
    setInputValues((prev) => ({
      ...prev,
        ...
        prevLists.map((list) => {
          ...
          return {
            ...
            items: list.items.map((item) => {
              ...
              const nuevaCantidad = Math.max(0, Math.min(item.cantidad_meta, item.cantidad_cargada + delta));
  ...
  const handleReset = async (itemId) => {
    if (!window.confirm('¿Deseas reiniciar el total cargado de este ítem?')) return;
  ...
  const getListProgress = (list) => {
    ...
    const completed = list.items.filter(
      (item) => item.cantidad_meta > 0 && item.cantidad_cargada >= item.cantidad_meta
    ).length;
  ...
  const getStatusBadge = (list) => {
    ...
    if (list.items.length === 0) return { text: 'Sin ítems', className: 'bg-secondary' };
    if (completed === list.items.length) return { text: 'Completado', className: 'bg-success' };
    if (percentage >= 50) return { text: 'En progreso', className: 'bg-info text-dark' };
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #eef2ff 100%)', minHeight: 'calc(100vh - 120px)' }}
    >
      <div className="d-flex flex-column flex-lg-row gap-3">
        <div className="flex-shrink-0" style={{ minWidth: '300px', maxWidth: '340px' }}>
          <div
            className="card shadow-sm border-0 h-100"
            style={{ borderRadius: '18px', overflow: 'hidden', background: '#ffffff' }}
          >
            <div
            ...
            <div className="card-body p-0">
              {loading ? (
                ...
                <div className="p-3 text-center text-muted">Aún no hay listas. Crea la primera para comenzar.</div>
              ) : (
                        ...
                        <div className="d-flex justify-content-between align-items-center">
                          <div>
                            ...
                            <small className="text-muted">
                              {formatDate(list.fecha)} | {progress.completed}/{list.items.length} ítems
                            </small>
                          </div>
                          <span className={`badge rounded-pill ${badge.className}`}>{badge.text}</span>
                        </div>
                        <div className="progress progress-thin mt-2" style={{ height: '6px' }}>
                          <div
                            className="progress-bar"
                            role="progressbar"
                            style={{ width: `${progress.percentage}%`, background: 'linear-gradient(135deg, #0dcaf0 0%, #0d6efd 100%)' }}
        ...
        <div className="flex-grow-1">
          {!selectedList ? (
            <div className="alert alert-info shadow-sm">Selecciona una lista de carga o crea una nueva para comenzar.</div>
          ) : (
            <div
            ...
            >
              ...
              >
                <div>
                  ...
                  <div className="text-muted small">
                    ...
                    {selectedList.referencia ? ` • ${selectedList.referencia}` : ''}
                  </div>
                </div>
                <button
              ...
              <div className="card-body" style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #eef2ff 100%)' }}>
                {selectedList.items.length === 0 ? (
                  <div className="text-center text-muted py-5">
                    <i className="bi bi-archive fs-1 d-block mb-2"></i>
                    Aún no hay ítems en esta lista. Utiliza “Agregar ítem” para comenzar.
                  </div>
                ) : (
                  <div className="vstack gap-3">
                    {selectedList.items.map((item) => {
                      const remaining = Math.max(0, item.cantidad_meta - item.cantidad_cargada);
                      ...
                      const status =
                        item.cantidad_meta === 0
                          ...
                          ? { text: 'Casi listo', className: 'bg-info text-dark' }
                          : { text: 'Pendiente', className: 'bg-warning text-dark' };

                      const history = historyMap[item.id] || [];
                      const isHistoryLoading = historyLoading[item.id];

                      return (
                        <div key={item.id} className="card border-0 shadow-sm" style={{ borderRadius: '16px', overflow: 'hidden' }}>
                          <div
                            className="card-body"
                            style={{
                          ...
                          >
                            <div className="d-flex flex-column flex-lg-row justify-content-between gap-3">
                              <div>
                                <div className="d-flex align-items-center gap-3 flex-wrap">
                                  ...
                                  <span className={`badge rounded-pill ${status.className}`} style={{ fontSize: '0.75rem' }}>
                                    {status.text}
                                  </span>
                                </div>
                                <div className="text-muted small mt-1">
                                  Meta: <strong>{formatNumber(item.cantidad_meta)}</strong> • Cargado:{' '}
                                  <strong>{formatNumber(item.cantidad_cargada)}</strong> • Restante:{' '}
                              ...
                              <div className="d-flex flex-column flex-md-row align-items-stretch gap-2 flex-wrap">
                                <input
                                  ...
                                  style={{ maxWidth: '110px', borderRadius: '12px' }}
                                />
                                ...
                                <button className="btn btn-sm btn-outline-dark fw-semibold" onClick={() => handleReset(item.id)}>
                                  Reset
                                </button>
                              </div>
                            </div>
                            ...
                            {historyExpanded && (
                              <div className="mt-3 border-top pt-3">
                                <h6 className="small text-uppercase text-muted mb-2">Historial</h6>
                                {isHistoryLoading ? (
                                  <div className="text-muted small">Cargando historial...</div>
                                ) : history.length === 0 ? (
                                  <div className="text-muted small">Aún no hay movimientos registrados.</div>
                                ) : (
                                    ...
                                    {history.map((entry) => (
                                      <li key={entry.id} className="mb-1">
                                        ...
                                        <span className="text-muted">
                                          ...
                                          {new Date(entry.fecha_evento).toLocaleTimeString('es-VE')})
                                        </span>
                                      </li>
                                    ))}
                                  </ul>
      ...
      <Modal
        ...
        footer={
          <>
            <button className="btn btn-outline-secondary" onClick={() => setShowListModal(false)} disabled={saving}>
              Cancelar
            </button>
            <button className="btn btn-primary" onClick={handleCreateList} disabled={saving}>
        ...
        <form onSubmit={handleCreateList}>
          ...
          <div className="mb-3">
            ...
            <input
              ...
              onChange={(e) => setListForm((prev) => ({ ...prev, fecha: e.target.value }))}
              required
            />
          </div>
          <div className="mb-0">
            ...
            <textarea
              ...
              onChange={(e) => setListForm((prev) => ({ ...prev, referencia: e.target.value }))}
              rows={2}
            />
          </div>
        </form>
      ...
      <Modal
        ...
        footer={
          <>
            <button className="btn btn-outline-secondary" onClick={() => setShowItemModal(false)} disabled={saving}>
              Cancelar
            </button>
            <button className="btn btn-primary" onClick={handleCreateItem} disabled={saving || !selectedListId}>
              {saving ? 'Guardando...' : 'Guardar'}
            </button>
          </>
        }
        ...
        <form onSubmit={handleCreateItem}>
          ...
          <div className="mb-0">
            ...
            <input
              ...
              onChange={(e) => setItemForm((prev) => ({ ...prev, cantidad_meta: e.target.value }))}
            />
          </div>
        </form>
      </Modal>
